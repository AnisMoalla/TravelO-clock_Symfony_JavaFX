...
class ReclamationFaccController extends AbstractController
{
    ...
    /**
     * @Route("/{id}", name="reclamation_facc_delete", methods={"DELETE"})
     */
    public function delete(Request $request, ReclamationFacc $reclamationFacc): Response
    {
        if ($this->isCsrfTokenValid('delete'.$reclamationFacc->getId(), $request->request->get('_token'))) {
            $entityManager = $this->getDoctrine()->getManager();
